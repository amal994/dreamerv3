...
class Agent(nj.Module):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    self.scales = scales # Mad : Config currently sets all scales to 1.0, apart from replay critic that the dup WM doesn't have
  ...
  def policy_keys(self):
    return '/(enc|dec|dyn|actor|dup_enc|dup_dec|dup_dyn)/'
  ...
  def rev_step(self, obs, alt_action, dup_carry):
    ...
    noop_action = jaxutils.onehot_dict({'action': np.array([0], dtype=np.int32)}, self.act_space)

    # We are getting the latents upon taking no (no-op) action in the current state
    # Using these latents we are making the reverse WM imagine taking the intended action
    # Aim is to get the reverse WM's prediction of what the previous state must have been, if the 
    # given action was used to get to the current state

    dup_obs_carry, _ = self.dup_dyn.observe(dup_carry[0], noop_action, dup_embed, obs['is_last'], bdims=1)

    _, dup_img_out = self.dup_dyn.imagine(dup_obs_carry, jaxutils.onehot_dict(alt_action, self.act_space), bdims=1)

    for k in dup_img_out.keys():
      dup_img_out[k] = dup_img_out[k][np.newaxis, :]
    decoded_img = self.dup_dec(dup_img_out, bdims=1)['image'].mode()

    return ({'image': decoded_img})
  
  def rev_step_with_consequences(self, obs, action, dup_carry):
    obs = self.preprocess(obs)
    dup_embed = self.dup_enc(obs, bdims=1)
    dup_obs_act = jaxutils.onehot_dict(action, self.act_space)

    if dup_carry[1]['action'].shape[-1] != self.act_space['action'].classes:
      prevact = jaxutils.onehot_dict(dup_carry[1], self.act_space)
    else:
      prevact = dup_carry[1]
    
    dup_obs_lat, _ = self.dup_dyn.observe(dup_carry[0], prevact, dup_embed, obs['is_last'], bdims=1)
    _, dup_img_out = self.dup_dyn.imagine(dup_obs_lat, dup_obs_act, bdims=1)
  ...
  def rev_step_in_one_go(self, obs, actions, dup_carry):
    ...
    for action in actions['action']:
      dup_act = jaxutils.onehot_dict({'action': jnp.array([action], dtype=np.int32)}, self.act_space)
      dup_lat, dup_img_out = self.dup_dyn.imagine(dup_lat, dup_act, bdims=1)

      for k in dup_img_out.keys():
        dup_img_out[k] = dup_img_out[k][np.newaxis, :]
      decoded_img = self.dup_dec(dup_img_out, bdims=1)['image'].mode()
      decoded_images = decoded_img if decoded_images is None else jnp.concatenate([decoded_images, decoded_img], axis = 0)
  ...
  def policy(self, obs, carry, mode='train'):
    ...
    if mode == 'eval':
      # We are printing the forward WMs prediction of the next state if no (no-op) action was 
      # taken in the current state. This is to have a baseline comparison of the quality of images 
      # generated by the reverse WM

      noop_action = jaxutils.onehot_dict({'action': np.array([0], dtype=np.int32)}, self.act_space)
      _, img_outs = self.dyn.imagine(lat, noop_action, bdims=1)
      img_outs['deter'] = img_outs['deter'][np.newaxis, :, :]
      img_outs['logit'] = img_outs['logit'][np.newaxis, :, :, :]
      img_outs['stoch'] = img_outs['stoch'][np.newaxis, :, :, :]
      decoded_img_image = self.dec(img_outs, bdims=1)['image'].mode()

      return act, outs, (lat, act), ({'image': decoded_img_image})
    else:
      return act, outs, (lat, act), None
